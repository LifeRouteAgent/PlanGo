...
from pydantic import (
    BaseModel,
    Field,
)  # BaseModel 用于定义请求/响应模型，Field 用于设置默认值、校验规则和字段说明
...
class TripPlanRequest(BaseModel):  # 行程规划接口的请求体模型
    ...
    )  # 用户画像信息，默认为空字典，例如城市、预算、偏好等
    max_replanning_count: int = Field(
        default=2, ge=0, le=3
    )  # 最大重规划次数，默认 2 次，范围 0 到 3
