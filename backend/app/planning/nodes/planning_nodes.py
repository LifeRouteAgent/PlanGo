...
def plan_editor_node(value: PlanningState | dict[str, Any]) -> dict[str, Any]:
    ...
    target = (
        ["restaurant"]
        if adjustment in {"replace_slot", "lower_budget"}
        else list(state.constraints.hard_constraints.required_slots)
    )
...
def route_planner_node(value: PlanningState | dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "plans": plans,
        "debug": append_trace(
            state, "route_planner", f"created {len(plans.candidate_plans)} candidate plans"
        ),
    }
