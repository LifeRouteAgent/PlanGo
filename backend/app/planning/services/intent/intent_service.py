...
from app.planning.services.intent.utils import (
    LEGACY_TO_LOGICAL,
    SLOT_CATEGORIES,
    MEAL_KEYWORDS,
    INSPIRATION_MUST_PATTERN,
...
def resolve_intent(
    ...
    *,
    ...
    return LLMUnderstanding(
        ...
        poi_keyword_intent=POIKeywordIntent(
        ...
        rating=RatingUnderstanding(min_rating=4.0, rating_preference="normal"),
    )
...
def _time_range_hours(message: str) -> tuple[float, float] | None:
    range_match = re.search(
        r"(?P<start>\d{1,2})(?:[:：]\d{1,2})?\s*点?.{0,4}[到至\-—~]\s*(?P<end>\d{1,2})(?:[:：]\d{1,2})?\s*点?",
        message,
    )
    if not range_match:
        return None
