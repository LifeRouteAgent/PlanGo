...
class SessionStore:
    ...
    def save_turn(
        ...
        *,
        ...
        turns = (
            existing.get("turns", []) if isinstance(existing.get("turns"), list) else []
        )  # 获取历史轮次列表，格式不对则用空列表
        ...
        turns.append({  # 向历史轮次中追加本轮摘要
            ...
            "selected_plan_id": (
                (response.get("selected_plan") or {}).get("id")
            ),  # 保存当前选中方案 ID
            "ranked_plan_count": len(response.get("ranked_plans", []) or []),  # 保存候选方案数量
            "created_at": time.time(),  # 保存本轮创建时间戳
        })

        latest_planning_state = existing.get(
            "latest_planning_state"
        )  # 读取最近一次真正“规划类”状态
        latest_planning_response = existing.get(
            "latest_planning_response"
        )  # 读取最近一次真正“规划类”响应
        latest_planning_query = existing.get(
            "latest_planning_query"
        )  # 读取最近一次真正“规划类”用户 query
...
def _is_planning_state(state: dict[str, Any], response: dict[str, Any]) -> bool:
    ...
    intent_type = str(
        state.get("intent_type") or response.get("intent_type") or ""
    )  # 从 state 或 response 中读取意图类型
    answer_mode = str(
        state.get("answer_mode") or response.get("answer_mode") or ""
    )  # 从 state 或 response 中读取回答模式

    if intent_type in {"simple_qa", "capability"} or answer_mode in {
        "simple_qa",
        "capability",
    }:  # 如果是简单问答或能力问答
